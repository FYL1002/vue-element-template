<template>
  <div class="filtered-search-box" :class="{true:'focus',false:''}[visiable.focusOnOrNot]">
    <div class="filtered-search-box-input-container droplab-dropdown">
      <div class="filtered-scroll-container">
        <div class="scroll-box">
          <div id="filtered-scroll-wrap" class="scroll-wrap">
            <ul id="filtered-scroll-width" class="tokens-container list-unstyled">
              <li v-for="(filterObj,i) in filteredObj" :key="i" class="filtered-search-token">
                <div
                  class="selectable"
                ...
                >
                  <div
                    ...
                    @click="editTypeFn(filterObj,i,$event)"
                  >{{ filterObj.oKey=='search'?filterObj.name:$t(filterObj.name) }}</div>
                  <div
                    v-if="filterObj.value.name"
                    class="value-container"
                    :class="{true:'selected',false:''}[visiable.editColorNum===i&&visiable.isEdit]"
                    @click="editFilteredFn(filterObj,i,$event)"
                  >
                    <div class="value">{{ $t(filterObj.value.name) }}</div>
                    <div class="remove-token inverted" @click="deleteOption(i,$event)">
          ...
          <div class="input-token" style="position:relative;">
            ...
            >
              <ul class="filter-dropdown">
                ...
                >
                  <button class="btn btn-link" type="button" @click="searchDataFn()">
                    <i class="el-icon-search" style="padding: 3px 5px 0 0;" />
                    <span>{{ $t('content.clickSearch') }}</span>
                  </button>
                </li>
                <li
                  v-for="(itemX,itemY,j) in searchOptionCopy"
                  :key="j"
                  class="filter-dropdown-item"
                ...
                >
                  ...
                  >
                    <span class="js-filter-tag dropdown-light-content">{{ $t(itemX.name) }}</span>
                  </button>
                </li>
              </ul>
            </div>
            <div
              v-show="visiable.secondDropBool"
              id="filterSecondMenu"
              class="filtered-search-input-dropdown-menu dropdown-menu"
              style="display:block;"
            >
              <ul v-if="searchOptionSave[visiable.typeName]">
                <li
                  v-for="(userItem,k) in searchOptionSave[visiable.typeName].dropItem"
                  :key="k"
                ...
                >
                  ...
                  >
                    <div class="dropdown-user-details">
                      <i v-if="userItem.showCheckIcon" class="fa fa-check-square-o checkFil" />
                      <span class="filPad">{{ $t(userItem.name) }}</span>
                    </div>
                  </button>
                </li>
              </ul>
    ...
    <button v-if="filteredObj.length!=0" class="clear-search" type="button">
      <i class="el-icon-close" @click="deleteAll()" />
    </button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    filterInputClick(event) {
      ...
      // document.querySelector("#filterFirstMenu").hide()
      // document.querySelector("#filterSecondMenu").hide()
      // angular.element("#filterFirstMenu").hide();
      // angular.element("#filterSecondMenu").hide();
      ...
      if ((filteredDataLen === 0 || this.filteredObj[this.visiable.editNum].oKey === 'search' || this.filteredObj[this.visiable.editNum].value || this.visiable.isEditType) && !this.visiable.isEdit) {
        ...
        // document.querySelector("#filterFirstMenu").show();
        ...
        if (this.searchOptionSave[this.visiable.typeName].dropItem && this.searchOptionSave[this.visiable.typeName].dropItem.length !== 0) {
          ...
          // document.querySelector("#filterSecondMenu").show();
          // 判断是否可多次选择
    ...
    scrollXtop() {
      setTimeout(function() {
        var filterScroll = document.getElementById('filtered-scroll-wrap');
        var filterScrollWidth = document.getElementById('filtered-scroll-width').clientWidth;

        filterScroll.scrollLeft = filterScrollWidth;
      }, 0);
    },
    contentClick() {
      // 输入框失去焦点 赋值
