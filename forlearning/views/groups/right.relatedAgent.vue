...
<script>
import { getGroupsUserUserList } from '@/api/groups';
...
export default {
  ...
  data() {
    ...
    return {
      rules: {
        groupIds: [{
          required: true,
          message: '客服姓名' + this.$t('noticeMsg.NotEmpty'),
          trigger: 'change'
        }]
      },
      dialogForm: {},
  ...
  methods: {
    ...
    remoteMethod(query) {
      if (query !== '') {
        this.loading = true;
        getGroupsUserUserList({ emailContains: query, orderByDesc: 'createdOn', LoginAfterStatus: 'AcceptChats', groupId: this.agentData.id, isSelected: false }).then(data => {
          this.loading = false;
          if (data.results) {
